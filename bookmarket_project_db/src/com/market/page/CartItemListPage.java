...
//		this.cm = cm;
		...
		//"도서ID", "도서명", "단가", "수량", "총가격"
		cartItem = cartDao.select(MainWindow.member.getMid().toUpperCase());
		Object[][] content = new Object[cartItem.size()][tableHeader.length];
		Integer totalPrice = 0;
		for (int i = 0; i < cartItem.size(); i++) {
			CartVo item = cartItem.get(i);
			...
			content[i][5] = item.getTotal_sprice();
...
//				else 
				if (mSelectRow == -1)
					...
					int result = cartDao.delete(cartItem.get(mSelectRow).getCid());
					if(result ==1) {
						showList();
						mSelectRow = -1;
					}
...
//				}else 
				if(mSelectRow == -1){
					...
					if(qty >= 0) {
						cartDao.update(cartItem.get(mSelectRow).getCid(), "plus");
						showList();
						mSelectRow = -1;
					} 
//					else {
...
//				}else 
				if(mSelectRow == -1){
					...
					int qty = cartItem.get(mSelectRow).getQty();
					if(qty > 0) {
						cartDao.update(cartItem.get(mSelectRow).getCid(), "minus");
						showList();
						mSelectRow = -1;
					} 
//					else {
